...
#execute if data entity @s Inventory[{Slot:-106b,tag:{magic_wand:1b}}] run say Press F with MS

execute if entity @s[tag=shield.enable] run function system:players/shield/disable
execute if entity @s[tag=!shield.enable] if entity @s[tag=!bypasser] if score @s player.shield matches 0 run function system:players/shield/enable_failed
execute if entity @s[tag=has_shield,tag=!shield.enable] if entity @s[tag=!bypasser] run function system:players/shield/enable
