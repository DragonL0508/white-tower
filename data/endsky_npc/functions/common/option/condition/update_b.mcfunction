...
execute if score $condition npc.state matches 0 if score $changed npc.look_option matches 1 run data modify entity @e[limit=1,type=minecraft:text_display,tag=npc.option_b,tag=npc.this] text set value '{"color":"gray","text":"","extra":[{"nbt":"data.Dialogue.Options[1].Option","entity":"@e[limit=1,type=minecraft:marker,tag=npc.this]","interpret":true}]}'
...
execute if score $condition npc.state matches 1 if score $changed npc.look_option matches 1 run data modify entity @e[limit=1,type=minecraft:text_display,tag=npc.option_b,tag=npc.this] text set value '{"color":"black","text":"","extra":[{"nbt":"data.Dialogue.Options[1].Option","entity":"@e[limit=1,type=minecraft:marker,tag=npc.this]","interpret":true}]}'
