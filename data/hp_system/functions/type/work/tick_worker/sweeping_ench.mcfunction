...
#加入統計傷害
execute if score @s hit_amount matches 2.. unless data entity @s SelectedItem.tag.Enchantments[{id:"minecraft:sweeping"}] if score is_looking hit_amount matches 1 run scoreboard players operation @s Fan.FinalDamage += @e[tag=find_looking.result,sort=nearest,limit=1] damage
...
#加入統計傷害
execute if score @s hit_amount matches 2.. unless data entity @s SelectedItem.tag.Enchantments[{id:"minecraft:sweeping"}] if score is_looking hit_amount matches 0 run scoreboard players operation @s Fan.FinalDamage += @e[tag=Fan.gotHurt,sort=nearest,limit=1] damage
